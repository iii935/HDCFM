...
def build_spatial_branch_ori(in_channels, kernel_size, head=1,
                         nonlinearity='relu',
                         stride=1, gen_kernel_size=1):
    if gen_kernel_size > 1:
        return nn.Sequential(
...
def build_channel_branch_ori(in_channels, kernel_size,
                         nonlinearity='relu', se_ratio=0.2):
    assert se_ratio > 0
    mid_channels = int(in_channels * se_ratio)
    return nn.Sequential(
...
class DDFPack(nn.Module):
    def __init__(self, in_channels, kernel_size=3, stride=1, dilation=1, head=1,
                 se_ratio=0.2, nonlinearity='relu', gen_kernel_size=1, kernel_combine='mul',version='f'):
...
class DDFPackTemporal(nn.Module):
    def __init__(self, in_channels, temporal_cond_c = 32, kernel_size=3, stride=1, dilation=1, head=1,
                 se_ratio=0.2, nonlinearity='relu', gen_kernel_size=1, kernel_combine='mul',version='f'):
        super(DDFPackTemporal, self).__init__()
        assert kernel_size > 1
        self.kernel_size = kernel_size
        self.version = version
        self.stride = stride
        ...
        self.temporal_branch = nn.Sequential(
                                    nn.Conv2d(temporal_cond_c, 16, 1),
                                    nn.ReLU(True),
                                    nn.Conv2d(16, in_channels * kernel_size ** 2, 1))

    def forward(self, x, tem_cond):
        b, c, h, w = x.shape
        g = self.head
        k = self.kernel_size
        s = self.stride
        channel_filter = self.channel_branch(x).reshape(b*g, c//g, k, k)
        temporal_filter = self.temporal_branch(tem_cond).reshape(b*g, c//g, k, k)
        spatial_filter = self.spatial_branch(x).reshape(b*g, -1, h//s, w//s)
        x = x.reshape(b*g, c//g, h, w)
        out = ddf(x, channel_filter * temporal_filter, spatial_filter,
                  self.kernel_size, self.dilation, self.stride, self.kernel_combine)
        return out.reshape(b, c, h//s, w//s)
    # def count_your_model(model, x, y):
    #     # your rule here
...
class DDFPackOri(nn.Module):
    def __init__(self, in_channels, kernel_size=3, stride=1, dilation=1, head=1,
                 se_ratio=0.2, nonlinearity='relu', gen_kernel_size=1, kernel_combine='mul',version='f'):
        super(DDFPackOri, self).__init__()
        assert kernel_size > 1
        self.kernel_size = kernel_size
        self.version = version
        self.stride = stride
        ...
        self.spatial_branch = build_spatial_branch_ori(
            in_channels, kernel_size, head, nonlinearity, stride, gen_kernel_size)

        self.channel_branch = build_channel_branch_ori(
            in_channels, kernel_size, nonlinearity, se_ratio)
...
class DDFPackTwo(nn.Module):
    def __init__(self, in_channels, kernel_size=3, stride=1, dilation=1, head=1,
                 se_ratio=0.2, nonlinearity='relu', gen_kernel_size=1, kernel_combine='mul',version='f'):
        super(DDFPackTwo, self).__init__()
        assert kernel_size > 1
        self.kernel_size = kernel_size
        self.version = version
        self.stride = stride
    ...
    def forward(self, x,fs):
        ...
        k = self.kernel_size
        s = self.stride
        channel_filter = self.channel_branch(fs).reshape(b*g, c//g, k, k)
        spatial_filter = self.spatial_branch(fs).reshape(b*g, -1, h//s, w//s)
        x = x.reshape(b*g, c//g, h, w)
        out = ddf(x, channel_filter, spatial_filter,
                  self.kernel_size, self.dilation, self.stride, self.kernel_combine)
        return out.reshape(b, c, h//s, w//s)
...
def build_channel_1x1(in_channels, kernel_size,
                         nonlinearity='relu', se_ratio=0.2):
    assert se_ratio > 0
    mid_channels = int(in_channels * se_ratio)
    out_channel=in_channels*in_channels
    return nn.Sequential(
        nn.AdaptiveAvgPool2d((1, 1)),
        nn.Conv2d(in_channels, mid_channels, 1),
        nn.ReLU(True),
...
def convsplit(x,w,training=True):
    w=w.view(x.shape[0],x.shape[1],x.shape[1],1,1)
    out=[]
    for i in range(x.shape[0]):
        timeout=F.conv2d(x[i:i+1,:,:,:],w[i],stride=1)
        out.append(timeout)
    allout=torch.cat(out,axis=0)
    if not training:
        del out
        
    return allout

class DDFPackThree(nn.Module):
    def __init__(self, in_channels, kernel_size=3, stride=1, dilation=1, head=1,
                 se_ratio=0.2, nonlinearity='relu', gen_kernel_size=1, kernel_combine='mul',version='f'):
        super(DDFPackThree, self).__init__()
        assert kernel_size > 1
        self.kernel_size = kernel_size
        self.version = version
        self.stride = stride
        ...
        self.channel_branch = build_channel_branch(
            in_channels, kernel_size, nonlinearity, se_ratio)
        self.channel1x1=build_channel_1x1(in_channels, kernel_size, nonlinearity, se_ratio)
    ...
    def forward(self, x):
        ...
        out = ddf(x, channel_filter, spatial_filter,
                  self.kernel_size, self.dilation, self.stride, self.kernel_combine)
        timeout=out.reshape(b, c, h//s, w//s)
        w=self.channel1x1(x)
        # w=F.relu(w,inplace=True)
        out = convsplit(timeout,w)
        if self.training:
            out=out+timeout
            out=F.relu(out,inplace=True)
        else:
            out=out.cpu()+timeout.cpu()
            out=F.relu(out,inplace=True)
            out=out.to(x.device)
        return out
    ...
    def forward(self, x, joint_x=None):
        ...
        for s_b, c_b in zip(self.spatial_branch, self.channel_branch):
            ...
            o = ddf(_x, channel_filter, spatial_filter,
                    self.kernel_size, self.dilation, 1, self.head, self.kernel_combine,version='f').type_as(x)
